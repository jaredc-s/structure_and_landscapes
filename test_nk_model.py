...
class TestNKModelFactory(TC):
    ...
    def test_non_consecutive_dependancies(self):
        model = self.factory.non_consecutive_dependancies(6, 2)
        deps = model.dependancy_lists
        self.assertEqual(6, len(deps))
    ...
    def test_consecutive_dependancies_multigene(self):
        model = self.factory.consecutive_dependancies_multigene(3, 4, 2, 3)
        deps = model.dependancy_lists
        self.assertEqual(3 * 4, len(deps))
        self.assertEqual(deps[0], [0, 1, 2, 3, 6, 9])
        self.assertEqual(deps[-1], [11, 9, 10, 2, 5, 8])

    def test_non_consecutive_dependancies_multigene(self):
        model = self.factory.non_consecutive_dependancies_multigene(6, 3, 2, 10)
        deps = model.dependancy_lists
